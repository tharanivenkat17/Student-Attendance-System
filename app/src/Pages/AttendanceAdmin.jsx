...
function AttendanceAdmin() {
  ...
  const fetchExistingData = async (selectedDate) => {
    try {
      ...
      const existing = response.data.find(entry => entry);

      if (existing) {
        setExistingData(existing.student);
        setAttendance(existing.student); // Pre-fill checkboxes
        console.log('Fetched Attendance:', existing.student);
      } else {
        setExistingData({});
        setAttendance({});
      }
  ...
  //   try {
  //     const response = await axios.get(`${AttendanceData}?date=${selectedDate}`);
  //     const submitted = response.data.find(entry => entry.isAttendance === true);

  //     if (submitted) {
  //       setExistingData(submitted.student);
  //       console.log('Submitted Attendance:', submitted.student);
  //     } else {
  //       setExistingData({});
  //     }
  //   } catch (error) {
  ...
  const onSubmit = async () => {
    ...
    // Find the existing unsaved (isAttendance: false) entry for that date
    try {
      const response = await axios.get(`${AttendanceData}?date=${date}`);
      const existing = response.data.find(entry => entry);

      if (existing && !existing.isAttendance) {
        const updatedData = {
          ...existing,
          student: { ...attendance },
          isAttendance: true
        ...
        await axios.put(`${AttendanceData}/${existing.id}`, updatedData);
        ...
        await axios.post(`${AttendanceData}`, newEntry);
        alert(`Attendance submitted for ${FormatDate(date)}`);
        setAttendance({});
        reset();
        window.location.reload();
